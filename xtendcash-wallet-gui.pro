...
# cleaning "auto-generated" bitxtendcash directory on "make distclean"
...
# translation stuff
TRANSLATIONS = $$files($$PWD/translations/xtendcash-core_*.ts)
...
android{
    deploy.commands += make install INSTALL_ROOT=$$DESTDIR && androiddeployqt --input android-libxtendcash-wallet-gui.so-deployment-settings.json --output $$DESTDIR --deployment bundled --android-platform android-21 --jdk /usr/lib/jvm/java-8-openjdk-amd64 -qmldir=$$PWD
}
...
DISTFILES += \
    notes.txt \
    xtendcash/src/wallet/CMakeLists.txt \
