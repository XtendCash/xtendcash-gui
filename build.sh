...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/xtendcash-wallet-gui.app/Contents/MacOS/
...
cd build
$QMAKE ../xtendcash-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy xtendcashd to bin folder
...
cp xtendcash_default_settings.ini build/$BIN_PATH/xtendcash.ini

if [ "$platform" == "darwin" ]; then
    otool -l build/$BIN_PATH/xtendcash-wallet-gui | grep sdk
fi
